...
bool Coloring::findIndepCliques(std::vector<std::vector<Vertex> > &indClq, bool uncolored, bool inNoOtherClique){
  ...
  while(toVisit > 0){
	...
	if(!findMaxClique(tmp, uncolored, inNoOtherClique)){
		// clique of size 1
		break;
	}

	//store min clique in indep cliques
	if(tmp.size() < (unsigned) cl.vertInMinCl){
		cl.vertInMinCl = tmp.size();
	}
...
bool Coloring::findMaxClique(std::vector<Vertex> &clq, bool uncolored, bool inNoOtherClique){
  ...
  for(tie(vIt1,vIt2) = vertices(g); vIt1 != vIt2; vIt1++){
    ...
    if(tmpClique.size() > maxClique.size() && tmpClique.size() >= 2){
      maxClique = tmpClique;
    }
  }
